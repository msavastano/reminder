...
// Ported from frontend/src/lib/caregiverApi.ts.
...
export const patientLinksApi = {
  listCaregivers: () =>
    api.get<{ caregivers: LinkedCaregiver[] }>("/patient/caregivers").then((r) => r.caregivers),
  listInvites: () => api.get<{ invites: CaregiverInvite[] }>("/patient/invites").then((r) => r.invites),
  acceptInvite: (inviteId: string) =>
    api.post<{ caregiver: LinkedCaregiver }>(`/patient/invites/${inviteId}/accept`),
